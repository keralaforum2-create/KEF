...
export async function registerRoutes(
  ...
  // Startup Clinic registrations endpoints
  app.post("/api/startup-clinic", async (req, res) => {
    try {
      const result = insertStartupClinicSchema.safeParse(req.body);
      ...
      const registration = await storage.createStartupClinicRegistration(result.data);
      ...
      console.error("Error creating startup clinic registration:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });

  app.get("/api/startup-clinic", async (req, res) => {
    try {
      const registrations = await storage.getStartupClinicRegistrations();
      return res.json(registrations);
    } catch (error) {
      console.error("Error fetching startup clinic registrations:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });
  ...
  app.delete("/api/startup-clinic/:id", async (req, res) => {
    try {
      ...
      await storage.deleteStartupClinicRegistration(req.params.id);
      return res.json({ message: "Startup clinic registration deleted" });
    } catch (error) {
      console.error("Error deleting startup clinic registration:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });

  // Verify ticket number in registrations
  app.get("/api/verify-ticket/:ticketNumber", async (req, res) => {
    try {
      const { ticketNumber } = req.params;
      const registration = await storage.getRegistrationByRegistrationId(ticketNumber);
  ...
  app.post("/api/admin/register", async (req: Request, res) => {
    try {
      ...
      const baseUrl = resolveBaseUrl(req);
      const paidRegistrations = await storage.getPaidRegistrations();
      const registrationNumber = paidRegistrations.length;
  ...
  app.post("/api/phonepe/callback", async (req: Request, res) => {
    try {
      ...
      if (paymentStatus === 'paid') {
        ...
        const phonepePaidRegs = await storage.getPaidRegistrations();
        const phonepeRegNumber = phonepePaidRegs.length;
    ...
    try {
      ...
      if (statusResult.success && statusResult.status === 'SUCCESS') {
        ...
        if (registration.registrationType === 'speaker') {
          ...
          // Send admin notification with referral code info (count only paid registrations)
          const razorpayPaidRegs = await storage.getPaidRegistrations();
          const razorpayRegNumber = razorpayPaidRegs.length;
  ...
  app.post("/api/razorpay/verify", async (req: Request, res) => {
    try {
      ...
      // Send admin notification with referral code info (count only paid registrations)
      const razorpayVerifyPaidRegs = await storage.getPaidRegistrations();
      const razorpayVerifyRegNumber = razorpayVerifyPaidRegs.length;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const speakerPaidRegs = await storage.getPaidRegistrations();
          const speakerRegNumber = speakerPaidRegs.length;
